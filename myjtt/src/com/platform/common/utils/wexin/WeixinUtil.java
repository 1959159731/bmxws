...
public class WeixinUtil {
	...
	public static String getOpenid(HttpServletRequest request) {
		return StringUtils.isBlank(request.getParameter("openid")) ? (getSessionOpenid(request))
	...
	public static String getSessionOpenid(HttpServletRequest request) {
		...
		return session.getAttribute(WeixinConstans.USER_OPENID) == null ? null
				: session.getAttribute(WeixinConstans.USER_OPENID).toString();
	}
	...
	public String getAccessToken() throws Exception {
		WxToken token = wxTokenDao.get(WeixinConstans.ACCESS_TOKEN_SQL_ID);
		String accessToken = token.getReqTime() > new Date().getTime() ? token.getWxToken() : "";
		if (null == accessToken || StringUtils.isBlank(accessToken)) {
			String requestUrl = WeixinUtil.ACCESS_TOKEN_URL.replace("APPID", WeixinConstans.WEIXIN_APP_ID).replace("APPSECRET",
					WeixinConstans.WEIXIN_APP_SECRET);
			JSONObject jsonObject = WeixinUtil.httpRequest(requestUrl, "GET", null);
			if (jsonObject != null) {
				accessToken = jsonObject.getString("access_token");
				token.setWxToken(accessToken);
