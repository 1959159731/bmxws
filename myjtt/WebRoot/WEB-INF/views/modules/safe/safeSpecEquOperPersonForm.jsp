	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#btnSubmit").on("click", function(){
				$("#inputForm").attr("action", "${ctx}/safe/safeSpecEquOperPerson/save");
			});
			
			
			$("#exitSubmit").on("click", function(){
				$("#inputForm").attr("action", "${ctx}/safe/safeSpecEquOperPerson/exit");
			});
		});
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/safe/safeSpecEquOperPerson/">特种设备操作人员列表</a></li>
		<li class="active"><a href="${ctx}/safe/safeSpecEquOperPerson/form?id=${safeSpecEquOperPerson.id}">特种设备操作人员<shiro:hasPermission name="safe:safeSpecEquOperPerson:edit">${not empty safeSpecEquOperPerson.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="safe:safeSpecEquOperPerson:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="safeSpecEquOperPerson" action="" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="certificateValid" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate required"
					value="<fmt:formatDate value="${safeSpecEquOperPerson.certificateValid}" pattern="yyyy-MM-dd"/>"
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="approachTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate required"
					value="<fmt:formatDate value="${safeSpecEquOperPerson.certificateValid}" pattern="yyyy-MM"/>"
		...
		<div class="form-actions">
			<c:if test="${empty safeSpecEquOperPerson.exitTime }">
				...
				<c:if test="${not empty safeSpecEquOperPerson.id }">
					<shiro:hasPermission name="safe:safeSpecEquOperPerson:exit">
						<input id="exitSubmit" class="btn btn-primary" type="submit" value="退场"/>&nbsp;
					</shiro:hasPermission>
				</c:if>
			</c:if>
