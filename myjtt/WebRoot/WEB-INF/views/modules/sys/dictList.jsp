	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>描述</th>
				<th>排序</th>
				<shiro:hasPermission name="sys:dict:edit"><th style="text-align: center">操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="dict">
			<tr>
				...
				<shiro:hasPermission name="sys:dict:edit"><td style="text-align: center">
    				<a href="${ctx}/sys/dict/form?id=${dict.id}" title="编辑"><i class="icon-edit"></i></a>&nbsp;&nbsp;
					<a href="${ctx}/sys/dict/delete?id=${dict.id}&type=${dict.type}" onclick="return confirmx('确认要删除该字典吗？', this.href)" title="删除"><i class="icon-remove"></i></a>&nbsp;&nbsp;
    				<a href="<c:url value='${fns:getAdminPath()}/sys/dict/form?type=${dict.type}&sort=${dict.sort+10}'>
    					<c:param name='description' value='${dict.description}'/></c:url>" title="添加键值"><i class="icon-plus"></i></a>
