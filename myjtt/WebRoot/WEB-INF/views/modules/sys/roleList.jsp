			...
			<tr>
				...
				<shiro:hasPermission name="sys:role:edit"><td style="text-align: center">
					...
					<c:if test="${(role.sysData eq fns:getDictValue('是', 'yes_no', '1') && fns:getUser().admin)||!(role.sysData eq fns:getDictValue('是', 'yes_no', '1'))}">
						<a href="${ctx}/sys/role/form?id=${role.id}" title="编辑"><i class="icon-edit"></i></a>&nbsp;&nbsp;
					</c:if>
					<a href="${ctx}/sys/role/delete?id=${role.id}" onclick="return confirmx('确认要删除该角色吗？', this.href)" title="删除"><i class="icon-remove"></i></a>&nbsp;&nbsp;
