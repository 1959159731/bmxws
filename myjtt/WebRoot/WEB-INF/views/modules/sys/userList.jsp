...
<body>
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/sys/user/import" method="post" enctype="multipart/form-data" class="form-search"
			...
			<a href="${ctx}/sys/user/import/template" class="btn btn-success">下载模板</a>
		</form>
	</div>
		...
		<tbody>
			<c:forEach items="${page.list}" var="user">
				<tr>
					...
					<td>${user.mobile}</td>
					<shiro:hasPermission name="sys:user:edit">
						<td style="text-align: center"><a href="${ctx}/sys/user/form?id=${user.id}" title="编辑"><i class="icon-edit"></i></a>&nbsp;&nbsp;
						<a href="${ctx}/sys/user/delete?id=${user.id}" onclick="return confirmx('确认要删除该用户吗？', this.href)" title="删除"><i class="icon-remove"></i></a>
					</shiro:hasPermission>
				</tr>
			</c:forEach>
		</tbody>
